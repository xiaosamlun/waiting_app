...
define(function(require, exports, module) {
    ...
    PageView.prototype.createHeader = function(){
        ...
        this.headerContent.Invite.on('click', function(){
            ...
            Utils.Popover.Share({
                text: 'Share your profile to connect with people!',
                email: {
                    subject: encodeURI(App.Credentials.app_name),
                    body: encodeURI('View my '+App.Credentials.app_name+' profile and connect with me by visiting '+App.Credentials.public_url+'/u/' + App.Data.User.get('_id'))
                },
                twitter: {
                    text: encodeURI('View my '+App.Credentials.app_name+' profile and connect with me by visiting '+App.Credentials.public_url+'/u/' + App.Data.User.get('_id')).replace(/\#/g, "%23")
                },
                facebook: {
                    link: 'http://'+App.Credentials.public_url+'/u/' + App.Data.User.get('_id')
                },
                // linkedin: {
        ...
        this.headerContent.QuickInvite.on('click', function(){
            ...
            newRCode.populated().then(function(){
                ...
                Utils.Popover.Buttons({
                    ...
                    text: 'Give the unique code <strong>'+S(newRCode.get('code'))+'</strong> to another '+App.Credentials.app_name+' user who you want to connect with.',
                    buttons: [{
                        text: 'Send via SMS',
                        success: function(){
                            var sentence = "Try out OddJob! I'm on it now. "+App.Credentials.public_url+"/i/" + newRCode.get('code');
                        ...
                        success: function(){
                            ...
                            var sentence = "Try out "+App.Credentials.app_name+"! I'm on it now. "+App.Credentials.public_url+"/i/" + newRCode.get('code');
                            window.plugins.socialsharing.shareViaEmail(sentence, 'Join me on '+App.Credentials.app_name+'!', null, null, null, null, function(msg) {console.log('ok: ' + msg)}, function(msg) {Utils.Notification.Toast('error: ' + msg)})
                        }
                    },{
                        text: 'Copy to Clipboard',
                        success: function(){
                            Utils.Clipboard.copyTo('Try out '+App.Credentials.app_name+' at '+App.Credentials.public_url+'/i/' + newRCode.get('code'));
                        }
                    }]
                });
        ...
        this.headerContent.Share.on('click', function(){
            ...
            .then(function(email){
                ...
                // Valid email?
                Utils.Notification.Toast('Sending Email');
                ...
                .fail(function(){
                    Utils.Popover.Confirm('Failed sending invite!','Try Again','Cancel')
                    .then(function(result){
                        ...
                        Timer.setTimeout(function(){
                            that.lastEmail = email;
                            that.headerContent.Share.emit('click');
                        },250);
    ...
    PageView.prototype.createContent = function(){
        ...
        this.contentScrollView = new FlexibleLayout({
            ...
            ratios: [1] // true, 1 (if included tab bar)
        });
